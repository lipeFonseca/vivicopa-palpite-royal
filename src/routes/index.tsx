          ...
          <div className="space-y-4">
            {porJogo.map(({ jogoId, palpites: ps }) => {
              ...
              return (
                <div
                  key={jogoId}
                  className="rounded-xl border border-border bg-white/60 overflow-hidden"
                >
                  <div
...
function CommentReplyNotificationsMenu({
  notifications,
  unreadCount,
  onOpenGame,
  onChanged,
          ...
          return (
            <div key={p.id} className="rounded-2xl border border-border bg-card p-4 shadow-card">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-semibold text-muted-foreground">
                ...
                <span>
                  {p.respostasCount} resposta{p.respostasCount === 1 ? "" : "s"}
                </span>
                {p.parentId && <span>Resposta</span>}
              </div>
            </div>
          );
        })}
        {lista.length === 0 && (
          <div className="py-10 text-center text-muted-foreground">Nenhum comentário ainda.</div>
        )}
      </div>
    </div>
  );
